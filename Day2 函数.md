...
### 函数参数核心规则总结（10 题考点全覆盖）

1. **位置参数**：按「定义顺序」匹配，数量必须一致；
2. **关键字参数**：按「参数名」匹配，与传入顺序无关，可覆盖位置顺序；
3. **默认参数**：仅未传入时生效，**避免用可变类型（列表 / 字典）** 作为默认值；
4. `*args`：接收「剩余位置参数」，打包为元组；`**kwargs`：接收「剩余关键字参数」，打包为字典；
5. **参数解包**：`*` 解包列表 / 元组为位置参数，`**` 解包字典为关键字参数；
6. **参数组合顺序**：固定位置参数 → `*args` → 默认参数 → `**kwargs`（核心优先级）。